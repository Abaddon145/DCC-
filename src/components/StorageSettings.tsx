...
import { applyGlobalPreferences, commandRegistry, defaultGlobalPreferences, defaultLibraryPreferences, isHexColor, moduleGroupLabels, moduleRegistry, shortcutConflicts } from "../lib/personalization";
...
export function StorageSettings({ onClose, onLibraryChanged, notify, libraryLocked = false, initialTab = "online", onPersonalizationChanged }: Props) {
  ...
  const moveModule = (target: string) => {
    ...
    const sourceGroup = moduleRegistry.find(item => item.id === dragModule)?.group;
    const targetGroup = moduleRegistry.find(item => item.id === target)?.group;
    if (!sourceGroup || sourceGroup !== targetGroup) { setDragModule(null); return; }
    ...
    <div className="settings-center-layout">
      ...
      <div className="settings-body settings-page">
        {tab === "appearance" && <><SettingsTitle title="外观" text="主题、强调色与整体界面尺寸会实时预览。" />
          ...
          <section><h3>布局与动效</h3><SelectRow label="界面密度" value={global.density} onChange={value => updateGlobal("density", value as GlobalPreferences["density"])} options={[["comfortable","舒适"],["compact","紧凑"]]} /><RangeRow label="字号比例（%）" value={Math.round(global.fontScale * 100)} min={90} max={125} unit="%" onChange={value => updateGlobal("fontScale", value / 100)} /><SelectRow label="工具栏间距" value={global.toolbarDensity} onChange={value => updateGlobal("toolbarDensity", value as GlobalPreferences["toolbarDensity"])} options={[["comfortable","舒适"],["compact","紧凑"]]} /><RangeRow label="侧栏宽度" value={global.sidebarWidth} min={210} max={360} onChange={value => updateGlobal("sidebarWidth", value)} /><RangeRow label="详情面板宽度" value={global.detailWidth} min={340} max={640} onChange={value => updateGlobal("detailWidth", value)} /><CheckRow label="减少动画" checked={global.reduceMotion} onChange={value => updateGlobal("reduceMotion", value)} /></section>
        </>}
        {tab === "view" && <><SettingsTitle title="素材视图" text="这些选项只影响当前素材库。" /><section><h3>显示方式</h3><div className="segmented setting-segment"><button className={library.assetView === "grid" ? "active" : ""} onClick={() => updateLibrary("assetView", "grid")}><Grid2X2 size={16} />网格</button><button className={library.assetView === "list" ? "active" : ""} onClick={() => updateLibrary("assetView", "list")}><List size={16} />紧凑列表</button></div><SelectRow label="卡片尺寸" value={library.cardSize} onChange={value => updateLibrary("cardSize", value as LibraryPreferences["cardSize"])} options={[["small","小"],["medium","中"],["large","大"]]} /><SelectRow label="封面比例" value={library.coverAspectRatio} onChange={value => updateLibrary("coverAspectRatio", value as LibraryPreferences["coverAspectRatio"])} options={[["standard","标准"],["square","1:1 方形"],["wide","16:9 宽屏"]]} /><SelectRow label="封面适应" value={library.coverFit} onChange={value => updateLibrary("coverFit", value as LibraryPreferences["coverFit"])} options={[["cover","填满裁切"],["contain","完整显示"]]} /></section><section><h3>卡片信息</h3>{Object.entries({ category:"分类",tags:"标签",software:"软件",version:"版本",format:"格式",linkStatus:"链接状态" }).map(([key,label]) => <CheckRow key={key} label={label} checked={library.cardFields[key as keyof typeof library.cardFields]} onChange={value => updateLibrary("cardFields", { ...library.cardFields, [key]: value })} />)}</section><section><h3>导航默认展开</h3>{[["library","素材导航"],["imageLibrary","图片文件夹"],["modelLibrary","三维模型文件夹"],["audioLibrary","音频文件夹"],["videoLibrary","视频文件夹"]].map(([id,label]) => <CheckRow key={id} label={label} checked={!library.collapsedContextPanes.includes(id)} onChange={expanded => updateLibrary("collapsedContextPanes", expanded ? library.collapsedContextPanes.filter(value => value !== id) : [...library.collapsedContextPanes,id])} />)}</section></>}
        {tab === "modules" && <><SettingsTitle title="功能模块" text="模块按内容、创作、素材上下文和管理分组；可在组内拖拽排序。关闭模块不会删除数据。" /><section><div className="module-groups">{(["content","creation","libraryContext","manage"] as const).map(group => <div className="module-setting-group" key={group}><h3>{moduleGroupLabels[group]}</h3><div className="module-list">{library.moduleOrder.filter(id => moduleRegistry.find(item => item.id === id)?.group === group).map(id => { const module = moduleRegistry.find(item => item.id === id); if (!module) return null; const locked = id === "library" || (id === "reference" && libraryLocked); const enabled = id === "library" || !library.disabledModules.includes(id); return <div key={id} draggable={!locked} onDragStart={() => setDragModule(id)} onDragEnd={() => setDragModule(null)} onDragOver={event => event.preventDefault()} onDrop={() => moveModule(id)}><span className="drag-handle">⋮⋮</span><div><strong>{module.label}</strong><small>{module.description}{id === "reference" && libraryLocked ? " · 悬浮窗口使用中" : ""}</small></div><button className={`module-toggle ${enabled ? "active" : ""}`} disabled={locked} onClick={() => updateLibrary("disabledModules", enabled ? [...library.disabledModules, id] : library.disabledModules.filter(value => value !== id))}><Power size={14} />{enabled ? "已启用" : "已关闭"}</button></div>; })}</div></div>)}</div></section></>}
        ...
        {tab === "online" && <><SettingsTitle title="在线服务" text="除翻译、Fab 和链接检查外，软件可以完全离线使用。" />
          ...
          <section><TranslationGlossaryManager notify={notify} /></section>
        </>}
...
function RangeRow({ label, value, min, max, onChange, unit = "px" }: { label: string; value: number; min: number; max: number; onChange: (value: number) => void; unit?: string }) { return <label className="setting-row"><span>{label}<small>{value}{unit}</small></span><input type="range" min={min} max={max} value={value} onChange={event => onChange(Number(event.target.value))} /></label>; }
